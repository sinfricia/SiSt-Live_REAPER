-- @description Apply actions to all open projects
-- @version 1.0.0
-- @author sinfricia
-- @changelog
--   Initial release
...
local function get_project_list()
    ...
    while true do
        ...
        if short_name:find("ROUTING") then
            project_list[#project_list].checked = false
        end
        i = i + 1
    end
end
...
local function setup_gui()
    ...
    gfx.init("Select Projects - Action: " .. reaper.kbd_getTextFromCmd(action_id, section_id), gfx_w, gfx_h, 0, x_pos,
        y_pos)
...
local function render_gui()
    ...
    gfx.drawstr("Select projects to apply the following action to:\n" .. reaper.kbd_getTextFromCmd(action_id, section_id))

    -- Draw checkboxes for each project
    y = y + 40 -- Starting position for projects
    ...
    if mouse_down and mouse_x > button_x and mouse_x < button_x + button_width
        ...
        if not mouse_handled then
            ...
            for _, project in ipairs(project_list) do
                if project.name:find("ROUTING") then
